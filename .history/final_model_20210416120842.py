                        ...
                        if face_flag:
                            ...
                            for i in range(known_face_encodings.shape[0]):
                                for j in range(feature.shape[0]):
                                    ...
                                    cv2.rectangle(im0, (int(xyxy[1]) + b1s[j][0], int(xyxy[0])+b1s[j][1]), (int(xyxy[1])+b1s[j][2], int(xyxy[0])+b1s[j][3]), (255,0,0), 2)
                            ...
                            for i in range(after_sort.shape[1]):
                                if int(match[after_sort[0][i]][i])==1:
                                    cv2.putText(im0, known_face_names[after_sort[0][i]]+"%.2f"%(result[after_sort[0][i]][i]), (int(xyxy[1])+b1s[i][0], int(xyxy[0])+b1s[i][1]),\
